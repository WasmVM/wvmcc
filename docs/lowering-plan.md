...
| `goto` | dispatch loop keyed on a function-wide pending-label id: arbitrary forward/backward/non-local jumps, chaining to outer dispatch contexts, overlapping forward ranges, and jumps *into* nested compounds/if/loop bodies (entry-state descent skips conditions/initializers per 6.2.4p6); simple same-level forward gotos lifted to `Block`+`Br`; jump into a *switch body* rejected |
