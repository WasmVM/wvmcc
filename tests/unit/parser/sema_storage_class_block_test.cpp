...
static int run_case(const std::string &src, const std::string &expect_substr) {
    const std::string fname = "temp_sema_storage_block.c";
    {
...
int main() {
    struct Case { std::string src; std::string expect; } cases[] = {
        { "void f() { static int g(); }\n", "function with block scope shall have no explicit storage-class" },
        { "void f() { register int g(); }\n", "function with block scope shall have no explicit storage-class" }
    };
    ...
    std::cout << "sema-storage-block: OK" << std::endl;
    return 0;
}
