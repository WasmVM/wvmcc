...
static int run_case(const std::string &src, const std::string &expect_substr, bool expect_present) {
    const std::string fname = "temp_sema_inline.c";
    {
...
int main() {
    struct Case { std::string src; std::string expect; bool present; } cases[] = {
        // inline + extern declarations without a definition should error
        { "inline int f(); extern int f();\n", "declared inline with external linkage must be defined", true },
        // inline definition in the TU should not error
        { "inline int g(); inline int g() {}\n", "declared inline with external linkage must be defined", false }
    };
    ...
    std::cout << "sema-inline: OK" << std::endl;
    return 0;
}
